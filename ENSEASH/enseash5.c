...
int main() {
    ...
    while (1) {
        ...
        *strchr(cmd, '\n') = '\0';
        ...
        // Ne pas autoriser les commandes complexes pour le moment
        if (strchr(cmd, ' ') != NULL) {
            printError("L'execution de commandes complexes n'est pas implémentée pour l'instant.");
            continue;
        }

        // Executer la commande en mesurant le temps
        timeDeltaMs = launchAndMesureTime(cmd, &status);
    }
